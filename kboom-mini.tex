...
Ontologies provide a cohesive representation of some domain or
knowledge, such as human anatomy or human disorders. One of the
characteristic features of an ontology is that relationships between
elements have precise logical interpretations, given by the Web
...
broad and deep coverage of the domain. In other areas (for example,
...
employs radically different classification from DOID. When previous
attempts have been made to unify these, the results are frequently
fragmentary. For example, MedGen\cite{coordinators2015database} includes OMIM and Orphanet, but many
of the entries, such as that for \emph{Hyperekplexia hereditary} lack
a classification. We therefore decided that this area was in need of
an automated approach to ontology merging.
...
prohibitive.  In order to further reduce the search space, we apply a
